...
const COLUMNS = (
	...
	const columnData = [
		{
			...
			render: (
				symbol,
				{ fullname = '', verified, basename, name, ...rest }
			) => {
				...
				return (
					<div className="coin-symbol-wrapper" onClick={handleLinkClick}>
	...
	columnData.push({
		title: (
			<div>
				Orderbook (<Link to="/admin/trade?tab=1&isViewTabs=true">View</Link>)
			</div>
		),
		children,
	});
...
class PairsSummary extends Component {
	...
	componentDidMount() {
		if (this.props.pairs.length && this.props.allPairs.length) {
			const pairs = this.props.allPairs.filter((data) =>
								...
								>
									<div className="d-flex justify-content-between">
										<div className="d-flex align-items-center">
											<Link
												to={`/admin/trade?tab=1&isViewTabs=true&isOpenPairModal=true&pairs=${pairs[0]}-${pairs[1]}`}
												className="mr-1"
											>
												<IconToolTip
													type="settings"
									...
									<div className="d-flex justify-content-between">
										...
										<div className="text-end">
											<div>
												(
												<Link
													to="/admin/trade?tab=1&isViewTabs=true"
													className="text-underline"
												>
													View
												</Link>
